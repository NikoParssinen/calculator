        ...
        for (auto commands: COMMANDS) {
            if (command == commands.str) {
                ...
                if (commands.exit == true) {
                    ...
                    if (pieces.size()-1 == commands.parameter_number) {

                        double i;
                        if (string_to_double(pieces.at(1), i) == false) {
                            cout << "Error: a non-number operand." << endl;
                        }
                        else if (string_to_double(pieces.at(2), i) == false) {
                            cout << "Error: a non-number operand." << endl;
                        }
                        else {
                            cout << commands.action(stod(pieces.at(1)), stod(pieces.at(2))) << endl;
